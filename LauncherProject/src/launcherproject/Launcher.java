...
public class Launcher {
    ...
    private static String SCHEMA_PATH = "/Users/admin/EBTfromGit/esb-benchmark-tool/scenario.xsd";
    ...
    public void runLauncher () {
        ...
        Boolean valid = false, resourcesAvailable = false;
        File xmlFile = new File(getInputFilePath());

        System.out.println("XML File exists? " + xmlFile.exists());
        if (xmlFile.exists()) {
            if (validateScenario (xmlFile) == true) {
                // Initialize the scenario from XML config file
                scenario = new Scenario(xmlFile.getAbsolutePath());
                resourcesAvailable = checkResourcesAvailability();
                System.out.println("Resources available ?" +resourcesAvailable);
